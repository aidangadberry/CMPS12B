...
void load_dictionary (const char *dictionary_name, hashset *hashset) {
   ...
   if (dictionary == NULL){
      print_error (dictionary_name, "No such dictionary");
      return;
   }

   for (int i=0; ; i++){
      char *line = fgets (buffer, sizeof (buffer), dictionary);
      if (line == NULL){
         break;
      }
      line = strchr (buffer, '\n');
      if (line == NULL){
         fflush (NULL);
         fprintf (stderr, "%s: %s[%d]: line not terminated error\n",
                  program_name, dictionary_name, i);
         fflush (NULL);
         exit_status = ERROR_DETECTED;
      }else {
         *line = '\0';
